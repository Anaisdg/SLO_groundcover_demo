...
LINEAR_MCP_URL = os.environ.get("LINEAR_MCP_URL", "https://mcp.linear.app/sse")
...
class MCPToolRouter:
    ...
    async def connect(self):
        ...
        print("[MCP] Connecting to Groundcover...", flush=True)
        gc_read, gc_write, _ = await self._exit_stack.enter_async_context(
            streamablehttp_client(GROUNDCOVER_MCP_URL, headers=self._gc_headers())
        )
        ...
        print("[MCP] Connecting to Linear...", flush=True)
        lin_read, lin_write = await self._exit_stack.enter_async_context(
            sse_client(LINEAR_MCP_URL, headers=self._linear_headers())
        )
    ...
    def _gc_headers(self) -> dict:
        ...
        key = os.environ.get("GROUNDCOVER_API_KEY", "")
        if key:
